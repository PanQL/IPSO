...
func (h *Handler) handleHintHotPathCond(channelID string, txid string, key string, cond pb.HotPathCondType, amount int) error {
	payloadBytes := marshalOrPanic(&pb.HotPathCond{Key: key, CondType: cond, Amount: int64(amount)})
	msg := &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_HINT_HOT_PATH_POINT, Payload: payloadBytes, Txid: txid, ChannelId: channelID}
	responseMsg, err := h.callPeerWithChaincodeMsg(msg, channelID, txid)
	if err != nil {
		return fmt.Errorf("error sending %s: %s", pb.ChaincodeMessage_HINT_HOT_PATH_POINT, err)
	}
