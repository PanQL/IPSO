...
type nsPubRwBuilder struct {
	...
	hotPathCond *peer.HotPathCond // is the hot path really executed?
}
...
func (b *RWSetBuilder) TracePath(ns string, hotPathCond *peer.HotPathCond) {
	nsPubRwBuilder := b.getOrCreateNsPubRwBuilder(ns)
	nsPubRwBuilder.tracePath(hotPathCond)
}
...
func (b *nsPubRwBuilder) tracePath(hotPathCond *peer.HotPathCond) {
	b.hotPathCond = hotPathCond
}
...
func (b *nsPubRwBuilder) buildDelta() *NsRwdSet {
	...
	if b.namespace == "" || b.namespace == "_lifecycle" || b.namespace == "lscc" {
		log.Printf("not build delta set for namespace %s", b.namespace)
		return &NsRwdSet{
			NameSpace: b.namespace,
		...
		deltaSet := []*kvrwset.KVDelta{}

		if b.hotPathCond != nil {
			log.Printf("build delta set for namespace %s", b.namespace)
			deltaSet, readSet, writeSet = CalculateDeltaFromRWset(readSet, writeSet)
			readSet = []*kvrwset.KVRead{} // PandaTODO: 目前只需要考虑SendPayment，所以readSet直接清空。
			// 未来则需要按照“某一个Read是否非线性地参与生成一个Write”来判断
		} else {
			log.Printf("not build delta set for namespace %s, hotPathTracer=%v", b.namespace, b.hotPathCond)
		}
		// log.Printf("build delta set for namespace %s", b.namespace)
