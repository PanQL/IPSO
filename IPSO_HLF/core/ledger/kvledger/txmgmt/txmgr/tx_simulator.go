...
func (s *txSimulator) Commit(txid string, simResult *ledger.TxSimulationResults) {
	rwdSet, err := rwsetutil.TxRwdSetFromProtoMsg(simResult.PubSimulationResults)
	if err != nil {
		panic(fmt.Sprintf("debug v5 TxRwdSetFromProtoMsg failed %v", err))
	}
	simResult.PubSimulationResults.ExecuteOrder = s.TempDB.Commit(txid, rwdSet)
}
...
// TracePath implements method in interface `ledger.TxSimulator`
func (s *txSimulator) TracePath(ns string, hotPathCond *peer.HotPathCond) {
	s.rwsetBuilder.TracePath(ns, hotPathCond)
}
