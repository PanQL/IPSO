...
func (h *Handler) handleMessageReadyState(msg *pb.ChaincodeMessage) error {
	...
	case pb.ChaincodeMessage_HINT_HOT_PATH_POINT:
		go h.HandleTransaction(msg, h.HandlePathTracing)
	default:
...
func (h *Handler) HandlePathTracing(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	chaincodeLogger.Debugf("Received %s in state %s", msg.Type, h.state)
	hotPathCond := &pb.HotPathCond{}
	err := proto.Unmarshal(msg.Payload, hotPathCond)
	if err != nil {
		return nil, errors.Wrap(err, "unmarshal failed")
	}

	txContext.TXSimulator.TracePath(txContext.NamespaceID, hotPathCond)
