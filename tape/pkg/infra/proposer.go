...
func (p *Proposer) Start() {
	for {
		...
		case element := <-p.inCh:
			...
			timeKeepers.keepProposedTime(element.Id, element.Txid, p.endorserIndex, p.connIndex, p.clientIndex)
			...
			if len(element.Responses) >= config.EndorserNum {
				...
				timeKeepers.keepEndorsedTime(element.Id, element.Txid, p.endorserIndex, p.connIndex, p.clientIndex)
			}
			element.lock.Unlock()
