...
import (
	...
	"github.com/PanQL/HLF-2.2/core/ledger/kvledger/txmgmt/rwsetutil"
	"github.com/PanQL/HLF-2.2/protoutil"
	"github.com/PanQL/fabric-protos-go/common"
	"github.com/PanQL/fabric-protos-go/orderer"
...
func checkResponsePayloadValidity(responses []*peer.ProposalResponse) error {
	...
	for i, r := range responses {
		...
		if bytes.Compare(payloadBytes, r.Payload) != 0 {
			if payloadBytes != nil {
				logger.Errorf("responses[0] from Peer %v", payloadBytes)
			} else {
				logger.Errorf("responses[0] from Peer nil")
			}
			if r.Payload != nil {
				logger.Errorf("responses[%d] from Peer %v", i, r.Payload)
			} else {
				logger.Errorf("responses[%d] from Peer nil", i)
			}
