...
type receiver struct {
	...
	abortedEndorseTxnBatch    []*cb.Envelope
}
...
func (r *receiver) scheduleMsg(msg *cb.Envelope) bool {
	...
	payload, err1 := utils.UnmarshalPayload(msg.GetPayload())
	if err1 != nil {
		panic("Can not get payload from the txn envelop")
	}
	chdr, err2 := utils.UnmarshalChannelHeader(payload.Header.ChannelHeader)
	if err2 != nil {
	...
	if ok := r.scheduler.ScheduleTxn(resppayload, r.blkHeight, txnID); ok {
		...
		chdr.Type = int32(cb.HeaderType_ORDER_ABORTED_TRANSACTION)
		chdr_bytes, _ := utils.Marshal(chdr)
		payload.Header.ChannelHeader = chdr_bytes
		payload_bytes, _ := utils.Marshal(payload)
...
func (r *receiver) Ordered(msg *cb.Envelope) (messageBatches [][]*cb.Envelope, pending bool) {
	...
	if r.scheduleMsg(msg) {
		...
		r.abortedEndorseTxnBatch = append(r.abortedEndorseTxnBatch, msg)
	}
	...
	// if len(r.abortedEndorseTxnBatch) >= blkSize {
	// 	messageBatch := r.abortedEndorseTxnBatch
	// 	r.abortedEndorseTxnBatch = make([]*cb.Envelope, 0)
	// 	messageBatches = append(messageBatches, messageBatch)
	// 	logger.Infof("# of aborted endorsement txns in cut batch %d, # of batches %d", len(messageBatch), len(messageBatches))
	// 	logger.Infof("================================================================\n")
	// }
...
func (r *receiver) Cut() []*cb.Envelope {
	...
	if len(schedule) > 0 {
		...
		batch = append(batch, r.abortedEndorseTxnBatch...)
		r.abortedEndorseTxnBatch = make([]*cb.Envelope, 0)
	}
